...
class CompendiaQuiz extends StatefulWidget {
  const CompendiaQuiz({super.key});

  @override
  State<CompendiaQuiz> createState() => _CompendiaQuizState();
}

class _CompendiaQuizState extends State<CompendiaQuiz> {
  List<MCQs> quizQuestions = [];
  RxInt currentQuestionIndex = 0.obs;
  RxMap<int, int?> userAnswers =
      <int, int?>{}.obs; // questionIndex to answerIndex
  RxBool isLoading = false.obs;
  RxBool isSubmitted = false.obs;
  String? compendiaTitle;
  ...
  void initData() {
    if (Get.arguments != null) {
      if (Get.arguments['mcqs'] != null) {
        final mcqsList = Get.arguments['mcqs'] as List<MCQs?>;
        quizQuestions = mcqsList.where((q) => q != null).cast<MCQs>().toList();
      }
      compendiaTitle = Get.arguments['title'];
    }
  }
  ...
  bool allQuestionsAnswered() {
    return quizQuestions.asMap().keys.every(
  ...
  void submitQuiz() {
    ...
    userAnswers.forEach((questionIndex, answerIndex) {
      if (answerIndex == quizQuestions[questionIndex].correctOptionIndex) {
        correctAnswers++;
      }
    });
      ...
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              "Percentage: ${(correctAnswers / quizQuestions.length * 100).toStringAsFixed(1)}%",
              style: TextStyle(
                fontSize: getWidth(16),
                fontWeight: FontWeight.w500,
              ),
    ...
    try {
      ...
      userAnswers.forEach((questionIndex, answerIndex) {
        if (questionIndex < quizQuestions.length && answerIndex != null) {
          answers.add({
            'questionId': quizQuestions[questionIndex].sId,
            'selectedOptionIndex': answerIndex,
          });
        }
      });
      final String? compendiaId = Get.arguments['compendiaId'];
      if (compendiaId != null) {
        Get.find<EthicalLearningController>().submitCompendiaQuiz(
          compendiaId: compendiaId,
          answers: answers,
        );
      }
    } catch (e) {}
  ...
  Widget build(BuildContext context) {
    return commonScaffold(
      ...
      appBar: commonAppBar(
        context: context,
        isBack: true,
        title: compendiaTitle != null ? "Quiz: $compendiaTitle" : "Quiz",
      ),
      body: Obx(() {
        if (isLoading.value) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        // Submit button (shows on last question or after all questions answered)
