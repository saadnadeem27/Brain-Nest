...
class TeacherUpdateAssignment extends StatefulWidget {
  const TeacherUpdateAssignment({Key? key}) : super(key: key);

  @override
  State<TeacherUpdateAssignment> createState() =>
      _TeacherUpdateAssignmentState();
}

class _TeacherUpdateAssignmentState extends State<TeacherUpdateAssignment> {
  ...
  late TeacherAssignmentModel assignment;
  ...
  void initData() {
    if (Get.arguments != null) {
      ...
      // Handle error case - no assignment provided
      Get.back();
      commonSnackBar(
        message: "Error: No assignment data found",
        color: Colors.red,
      );
    }
  }
  ...
  void _populateFormFields() {
    ...
    _lessonController.text = assignment.lesson ?? '';
    _contentsController.text = assignment.contents ?? '';
    _additionalInfoController.text = assignment.additionalInfo ?? '';
    _instructionsController.text = assignment.instructions ?? '';
    _submissionFormatController.text = assignment.submissionFormat ?? '';

    // Populate topics
    if (assignment.topics != null) {
      topics.addAll(assignment.topics!);
    }

    // Set due date
    if (assignment.dueDate != null) {
      try {
        dueDate.value = DateTime.parse(assignment.dueDate!);
      } catch (e) {
        log('Error parsing due date: $e');
    ...
    if (assignment.documents != null) {
      for (var doc in assignment.documents!) {
        documents.add({
          'type': doc.type ?? '',
          'link': doc.link ?? '',
          'name': doc.name ?? '',
    ...
    includeMCQ.value = assignment.isMCQ ?? false;

    if (includeMCQ.value && assignment.mCQs != null) {
      for (var mcq in assignment.mCQs!) {
        final optionControllers = List.generate(
          mcq.answerOptions?.length ?? 4,
          (index) =>
              TextEditingController(text: mcq.answerOptions?[index] ?? ''),
        );

        mcqQuestions.add(
          MCQQuestionItem(
            questionController: TextEditingController(text: mcq.question ?? ''),
            optionControllers: optionControllers,
            correctOptionIndex: mcq.correctOptionIndex ?? 0,
          ),
        );
      }

      numberOfQuestions.value = mcqQuestions.length;
    } else if (includeMCQ.value) {
      // Add default MCQ if none exists but isMCQ is true
      _addDefaultMCQ();
    }
  }
  ...
  Future<void> _updateAssignment() async {
    ...
    try {
      ...
      final result = await _controller.updateAssignment(
      ...
      () => commonScaffold(
        ...
        appBar: commonAppBar(
          ...
          actions: [
            // Delete button
            IconButton(
              onPressed: isDeleting.value ? null : _showDeleteConfirmation,
              icon:
                  isDeleting.value
                      ? SizedBox(
                        height: getHeight(20),
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed:
                                isSubmitting.value ? null : _updateAssignment,
                            child:
                                isSubmitting.value
                                    ? SizedBox(
                                      height: getHeight(20),
